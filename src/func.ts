...
const crawlLink = async (url: string) => {
  ...
  const i = usablePageIndex(); //TODO: 途中でPage0が使えなくなるっぽい
  ...
  const links = await page.$$eval('article a', (elms) => elms.map((elm) => elm.getAttribute('href')));
  const authors = await page.$$eval('.author-detail > a', (elms) => elms.map((elm) => elm.getAttribute('href')));
  const fetchedLinks = await Promise.all<linkDetail>(links.map((link) => fetchLink(link)));
...
const fetchLink = async (link: string | null): Promise<linkDetail> => {
  ...
  try {
    ...
    release();
    return { link, error: `${err.name}: ${err.code}` };
  }
};
